...
------------------------------------------------------------------------------------------------
--  ***** Preuve 1    z x conj (z) = z.re^2 + z.im^2
--- **** version 2 ( sans tactiques)
------------------------------------------------------------------------------------------------
theorem mul_conj_calc1 (z : ℂ) :
  z * conj z = (z.re^2 + z.im^2 : ℝ) := by
  -- On prouve l'égalité complexe par parties réelle et imaginaire
  apply Complex.ext
  · -- Partie réelle
...
theorem sub_conj_atomique2 (z : ℂ) : z - conj z = (2 * z.im : ℝ) * I :=
  Complex.ext
...
theorem four_mul_conj_detailed (z : ℂ) :
  ...
  calc
    4 * (z * conj z)
        = 4 * (z.re^2 + z.im^2 : ℝ) := by rw [mul_conj_calc1] --  PREUVE HROUZ mul_conj_calc1
